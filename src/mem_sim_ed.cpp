...
mem_sim_ed::mem_sim_ed(int _pages):
	mem_sim(_pages),
	last_load_page(0)
{
	printf("ED\n");
...
int mem_sim_ed::handle_page_event(priority_queue<event>&eventQueue, long stime){
	...
	if(!freePages.empty()){
		...
		visitTime[ppage] = 1/(1-C);// set to max val
		freePages.pop();
		requestQueue.pop();
	}else{
...
int mem_sim_ed::clock_tick(task_structure*current,priority_queue<event>&eventQueue, long stime){
	...
	if(current->addmap.find(pg) != current->addmap.end()){
		...
		// visitTime[current->addmap[pg]] = stime;
			...
			{
				if(i->first == pg){
					visitTime[i->second] = C*visitTime[i->second] + 1;
				}else{
					visitTime[i->second] *= C;
				}
			}
